...
Candidate candidate1 = new Candidate(
    new Person("Ivan", "Ivanov", new Address("Minsk", "Kolosa", 3, 3)),
    new SubjectScore[] { subjectScore1, subjectScore2 });

SubjectScore subjectScore1_1 = new SubjectScore("Math", 9);
SubjectScore subjectScore2_1 = new SubjectScore("Physics", 10);
Candidate candidate2 = new Candidate(
    new Person("Iva", "Pon", new Address("Minsk", "Kupaly", 30, 30)),
    new SubjectScore[] { subjectScore1_1, subjectScore2_1 });

SubjectScore subjectScore1_2 = new SubjectScore("Math", 5);
SubjectScore subjectScore2_2 = new SubjectScore("Physics", 6);
Candidate candidate3 = new Candidate(
    new Person("Lida", "Petrova", new Address("Minsk", "Kulinki", 98, 23)),
    new SubjectScore[] { subjectScore1_2, subjectScore2_2 });
...
Candidate candidate4 = new Candidate(
    new Person("Alex", "Belyj", new Address("Minsk", "Poliski", 30, 5)),
    new SubjectScore[] { subjectScore1_3, subjectScore2_3 });
...
Candidate candidate5 = new Candidate(
    new Person("Kostas", "Kruglyj", new Address("Minsk", "Zaluski", 44, 3)),
    new SubjectScore[] { subjectScore1_4, subjectScore2_4 });


Candidate[] candidatesArray = new[]
{
    candidate1,
    candidate2,
    candidate3,
...
for (int i = 0; i < candidatesArray.Length; i++)
{
    ...
    {
        if (subjectScoreArray[j].SubjectName == "Math" && subjectScoreArray[j].Score > maxScore)
        {
            maxScore = subjectScoreArray[j].Score;
        }
    }
}
Console.WriteLine("максимальная отметка по математике: " + maxScore);
